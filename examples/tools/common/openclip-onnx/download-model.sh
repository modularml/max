...
# Check to see if PyTorch, ONNX Runtime, and OpenCLIP are installed.
if python3 -c 'import torch; import onnxruntime; import open_clip' >&/dev/null; then
	python3 "$script" "$@"
else
	...
	"$venv_dir/bin/pip" install torch onnxruntime open_clip_torch
